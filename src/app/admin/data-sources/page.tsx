...
export default async function AdminDataSourcesPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-up max-w-6xl mx-auto pb-12">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-outline-variant/20">
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-secondary-container/30 bg-secondary-container/10 font-label-caps text-xs text-secondary-fixed-dim">
            ...
            OFFICIAL INTEGRATIONS TELEMETRY
          </div>
          ...
          <p className="text-on-surface-variant text-base max-w-2xl leading-relaxed">
            Real-time status probes across MoSPI, eSankhyiki, iGOT Karmayogi, and NSSTA APIs. Zero-trust validation protocol.
          </p>
        </div>
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h2 className="font-display text-lg font-bold text-on-surface">{src.source}</h2>
                  <span
                ...
                <div className="space-y-1 text-xs text-on-surface-variant">
                  <p>Last Sync: <strong className="text-on-surface font-mono">{src.lastSync ? new Date(src.lastSync).toLocaleString() : "—"}</strong></p>
                  <p>Last Successful: <strong className="text-on-surface font-mono">{src.lastSuccess ? new Date(src.lastSuccess).toLocaleString() : "—"}</strong></p>
                  <p>Records Synced: <strong className="text-primary font-mono">{src.recordCount}</strong></p>
                </div>
      ...
      {/* Provenance Footnote */}
      <div className="p-4 rounded-2xl bg-surface-container-high/40 border border-outline-variant/20 flex items-center justify-between text-xs text-on-surface-variant font-label-caps">
        <span>MoSPI eSankhyiki + Karmayogi Official Integrations</span>
        <span className="font-mono">Datasets: {officialRepo.listDatasets().length}</span>
      </div>
    </div>
  );
}

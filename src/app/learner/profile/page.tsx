          ...
          <div className="flex-1 space-y-2">
            ...
            {employee.careerGoal && (
              <p className="mt-4 text-on-surface-variant bg-surface-container-low px-4 py-3 rounded-xl border border-white/5 max-w-2xl">
                <strong className="text-on-surface block mb-1">Career Goal:</strong>
                {employee.careerGoal}
              </p>
            )}
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="glass-panel rounded-3xl p-8 space-y-6">
          <div className="flex items-center gap-3 border-b border-white/10 pb-4">
            ...
            <h3 className="font-display text-2xl font-bold text-on-surface">Career Path</h3>
          </div>
          
          <div className="space-y-6">
            <div>
              <p className="text-xs font-label-caps text-on-surface-variant mb-2">CURRENT DESIGNATION</p>
              <p className="text-lg font-medium text-on-surface">{employee.designation}</p>
            </div>
            
            <div className="pl-4 border-l-2 border-primary-container/30 py-2">
              <p className="text-xs font-label-caps text-on-surface-variant mb-2">TARGET ROLE</p>
              <p className="text-xl font-bold text-primary-container">{snap.targetRole?.name}</p>
            </div>
            
            <div className="bg-surface-container rounded-2xl p-6 border border-white/5">
              <div className="flex justify-between items-center mb-4">
                <p className="font-medium text-on-surface">Target Role Readiness</p>
        ...
        <div className="glass-panel rounded-3xl p-8 space-y-6">
          <div className="flex items-center gap-3 border-b border-white/10 pb-4">
            ...
            <h3 className="font-display text-2xl font-bold text-on-surface">Account Details</h3>
          </div>
          ...
          <ul className="space-y-4">
            <li className="flex justify-between items-center py-3 border-b border-white/5">
              ...
              <span className="px-3 py-1 rounded-full bg-primary-container/10 border border-primary-container/30 text-xs font-label-caps text-primary-container">ACTIVE</span>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-white/5">
              <span className="text-on-surface-variant">Auth Provider</span>
              <span className="text-on-surface font-medium">StatIQ Demo System</span>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-white/5">
              <span className="text-on-surface-variant">Total Learning Hours</span>
              <span className="text-on-surface font-medium">{snap.enrollments.reduce((s, e) => s + e.learningHours, 0)} hrs</span>
            </li>
            <li className="flex justify-between items-center py-3 border-b border-white/5">
              <span className="text-on-surface-variant">Competencies Tracked</span>
              <span className="text-on-surface font-medium">{snap.categoryScores.length} Domains</span>
            </li>
          </ul>
        </div>
      </div>

      <ProfileEditor currentEmail={session?.email || ""} />
    </div>
  );
}

...
export function TrainerCoursesView({
  courses,
  maps,
  documents,
  query,
}: TrainerCoursesViewProps) {
    ...
    <div className="space-y-8 animate-fade-up max-w-6xl mx-auto pb-12">
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-outline-variant/20">
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary-container/30 bg-primary-container/10 font-label-caps text-xs text-primary-container">
            ...
            {t("trainer.curriculumBadge", "CURRICULUM MANAGEMENT & RAG REPOSITORY")}
          </div>
          <h1 className="font-display text-4xl md:text-5xl font-bold text-on-surface tracking-tight">
            {t("trainer.coursesTitle", "Course Builder & Catalog")}
          </h1>
          <p className="text-on-surface-variant text-base max-w-2xl leading-relaxed">
            {t("trainer.coursesSubtitle", "Curate statistical training modules, map competencies to official DoPT standards, and index training materials for AI retrieval-augmented grounding.")}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          >
            ...
            {t("trainer.publicView", "Public View")}
          </Link>
          <button
            type="button"
          ...
          >
            ...
            {t("trainer.addNewCourse", "Add New Course")}
          </button>
        </div>
      </header>
      ...
      {query && (
        <div className="p-4 rounded-2xl bg-primary-container/10 border border-primary-container/30 flex items-center justify-between gap-3">
          ...
          >
            ...
            {t("trainer.clearFilter", "Clear Filter")}
          </Link>
        </div>
      )}
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display text-2xl font-bold text-on-surface">
              {t("trainer.mappedCourses", "Mapped Courses")} ({courses.length})
            </h2>
            <p className="text-xs text-on-surface-variant mt-0.5">
              {t("trainer.mappedCoursesDesc", "Modules actively recommended in learner skill gap pathways")}
            </p>
          </div>
        </div>
        ...
        {courses.length === 0 ? (
          <div className="glass-panel p-12 rounded-3xl border border-outline-variant/30 text-center space-y-3">
            ...
            <h4 className="font-display text-base font-bold text-on-surface">
              {t("trainer.noCoursesMatching", "No courses matching")} &ldquo;{query}&rdquo;
            </h4>
            <p className="text-xs text-on-surface-variant max-w-sm mx-auto">
              {t("trainer.trySearchingAnother", "Try searching with another competency keyword or clear the active filter.")}
            </p>
            <Link
              href="/trainer/courses"
            ...
            >
              {t("trainer.viewAllCourses", "View All Courses")}
            </Link>
          </div>
        ) : (
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[11px] font-label-caps text-on-surface-variant px-2.5 py-0.5 rounded-md bg-surface-container-high">
                        {tEntity(course.difficulty)} · {course.durationHours}h
                      </span>
                    </div>
                  ...
                  <div className="space-y-3 pt-3 border-t border-outline-variant/20">
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-label-caps text-on-surface-variant uppercase tracking-wider block">
                        {t("trainer.mappedCompetencies", "Mapped Competencies")}:
                      </span>
                    ...
                    <div className="flex items-center justify-between pt-1 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {t("trainer.viewCurriculum", "View Curriculum")}
      ...
      <section className="glass-panel p-6 md:p-8 rounded-3xl border border-outline-variant/30 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-outline-variant/20">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-display text-xl font-bold text-on-surface">
                {t("trainer.ragTitle", "Indexed Training Materials (RAG Knowledge Base)")}
              </h2>
            </div>
            <p className="text-xs text-on-surface-variant mt-1">
              {t("trainer.ragSubtitle", "Official PDF guidelines, NSSTA manuals, and MoSPI statistical standards indexed for AI Tutor and Quiz generation.")}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {t("trainer.uploadDocument", "Upload Document")}
          </button>
        </div>
          ...
          {documents.map((doc) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-on-surface">{tEntity(doc.title)}</p>
              ...
              <div className="flex items-center gap-3 self-end sm:self-auto">
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400 text-[10px] font-label-caps uppercase font-bold">
                  ...
                  {t("trainer.indexed", doc.status || "Indexed")}
                </span>
                <button
                  type="button"
                ...
                >
                  {t("trainer.viewEmbeddings", "View Embeddings")}
                </button>
              </div>
            </div>
          ))}

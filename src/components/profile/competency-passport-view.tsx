    ...
    <div className="space-y-10 animate-fade-up max-w-5xl mx-auto pb-16">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="font-display text-4xl md:text-5xl font-bold text-on-surface tracking-tight">
              {t("passport.title", "Competency Passport")}
            </h1>
            <p className="mt-2 max-w-2xl text-on-surface-variant text-lg leading-relaxed">
              {t("passport.subtitle", "Official verifiable record of statistical skills and verified competencies for")} {employee.name}.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <Link
              ...
              className="glow-button px-6 py-2.5 rounded-xl font-label-caps text-xs font-bold tracking-wider flex items-center gap-2 text-black"
            >
              <Sparkles className="w-4 h-4" />
              {t("assessments.newAssessment", "ELEVATE SCORES")}
            </Link>
          </div>
        </div>
      </header>
          ...
          <div className="p-6 rounded-2xl bg-surface-container-high/80 border border-white/10 flex items-center gap-6 backdrop-blur-md">
            <div>
              <p className="text-[10px] font-label-caps text-on-surface-variant uppercase tracking-wider">
                {t("learner.readinessScore", "TARGET ROLE READINESS")}
              </p>
      ...
      <div className="space-y-4">
        <h3 className="font-display text-2xl font-bold text-on-surface">
          {t("passport.filterDomain", "Domain Breakdown")}
        </h3>
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between">
          <div>
            <h3 className="font-display text-xl font-bold text-on-surface">
              {t("passport.competenciesTitle", "Official Competency Matrix")}
            </h3>
          ...
          <table className="w-full min-w-[700px] text-left text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {passport.map((item) => {
                ...
                const level = score >= 75 ? t("passport.level3", "ADVANCED") : score >= 50 ? t("passport.level2", "INTERMEDIATE") : t("passport.level1", "FOUNDATIONAL");
                  ...
                  <tr key={item.competencyId} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col items-end gap-1">
                        ...
                        <div className="w-20 h-1.5 rounded-full bg-surface-container-high overflow-hidden" dir="ltr">
                          <div 

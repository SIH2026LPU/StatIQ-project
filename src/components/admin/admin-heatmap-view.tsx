...
interface AdminHeatmapViewProps {
  heatmap: HeatmapRow[];
...
export function AdminHeatmapView({ heatmap, competencyNames }: AdminHeatmapViewProps) {
  const { t, tEntity } = useTranslation();

  const getStatusLabel = (score: number) => {
    if (score >= 75) return `${t("heatmap.proficient", "Proficient")} (≥75)`;
    if (score >= 50) return `${t("heatmap.developing", "Developing")} (50–74)`;
    if (score > 0) return `${t("heatmap.criticalDeficit", "Critical Deficit")} (<50)`;
    return t("heatmap.noData", "No Data Recorded");
  };
      ...
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2 border-b border-outline-variant/20">
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-primary-container/30 bg-primary-container/10 font-label-caps text-xs text-primary-container">
            ...
            {t("heatmap.badge", "WORKFORCE SKILL DENSITY MATRIX")}
          </div>
          <h1 className="font-display text-4xl md:text-5xl font-bold text-on-surface tracking-tight">
            {t("heatmap.title", "Competency Heatmap")}
          </h1>
          <p className="text-on-surface-variant text-base max-w-2xl leading-relaxed">
            {t("heatmap.subtitle", "Multi-dimensional matrix visualizing evidenced competency levels across all statistical departments and competency domains.")}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs font-label-caps text-on-surface-variant bg-surface-container-high/60 p-3 rounded-2xl border border-outline-variant/30">
          <span className="flex items-center gap-1.5 font-bold">
            ...
            <span className="text-emerald-900 dark:text-emerald-300">75+ ({t("heatmap.proficient", "Proficient")})</span>
          </span>
          <span className="flex items-center gap-1.5 font-bold">
            ...
            <span className="text-amber-900 dark:text-amber-300">50–74 ({t("heatmap.developing", "Developing")})</span>
          </span>
          <span className="flex items-center gap-1.5 font-bold">
            ...
            <span className="text-rose-900 dark:text-rose-300">&lt;50 ({t("heatmap.criticalDeficit", "Critical Deficit")})</span>
          </span>
        </div>
      </header>
        ...
        <div className="flex items-center justify-between pb-2">
          <div>
            <h2 className="font-display text-lg font-bold text-on-surface">
              {t("heatmap.matrixTitle", "Departmental Competency Matrix")}
            </h2>
            <p className="text-xs text-on-surface-variant">
              {t("heatmap.matrixDesc", "Hover over any cell to inspect division score, proficiency status, and benchmarks.")}
            </p>
          </div>
        </div>
                ...
                {competencyNames.map((name, idx) => (
                  <th
                    ...
                    className={`p-3 text-center font-display font-bold text-on-surface text-[11px] uppercase tracking-wider bg-surface-container-high/80 min-w-[120px] ${
                      idx === competencyNames.length - 1 ? "rounded-tr-xl" : ""
                    }`}
                  >
                    <span className="block text-center font-semibold" title={tEntity(name)}>
                      {tEntity(name)}
                    </span>
                  </th>
                ))}
              ...
              {heatmap.map((row) => (
                <tr key={row.department.id} className="hover:bg-white/5 transition-colors">
                  <td className="p-3.5 font-semibold text-on-surface text-xs whitespace-nowrap sticky left-0 bg-surface/95 backdrop-blur-sm z-10 border-r border-outline-variant/20">
                    <div className="font-bold text-on-surface">{tEntity(row.department.name)}</div>
                  ...
                  {row.competencies.map((c) => {
                    ...
                    const tooltip = `${tEntity(row.department.name)} · ${tEntity(c.name)}: ${
                      score ? `${score}/100 — ${getStatusLabel(score)}` : t("heatmap.noData", "No assessment data")
                    }`;

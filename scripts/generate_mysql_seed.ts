...
VALUES ('${orgId}', 'StatIQ National Analytics Organization', 'DEMO_ORG', NOW(), NOW())
...
VALUES
  ('user_admin_001', '${orgId}', 'admin@statiq.demo', '${hash}', 'ORG_ADMIN', 'Kavita Iyer', 'demoadmin', 'Kavita', 'Iyer', 'Kavita Iyer (Admin)', 'ACTIVE', 1, NOW(), NOW()),
  ('user_learner_001', '${orgId}', 'learner@statiq.demo', '${hash}', 'LEARNER', 'Aarav Sharma', 'aarav', 'Aarav', 'Sharma', 'Aarav Sharma', 'ACTIVE', 1, NOW(), NOW()),
  ('user_trainer_001', '${orgId}', 'trainer@statiq.demo', '${hash}', 'TRAINER', 'Dr. Ramesh Gupta', 'rgupta', 'Ramesh', 'Gupta', 'Dr. Ramesh Gupta', 'ACTIVE', 1, NOW(), NOW())
...
VALUES
  ('cat_stat_analysis', '${orgId}', 'Statistical Analysis & Inference', 'Methodologies, time series modeling, sample designs', NOW(), NOW()),
  ('cat_data_governance', '${orgId}', 'Data Governance & Ethics', 'Public data standards, metadata compliance, anonymization', NOW(), NOW()),
  ('cat_macro_indicators', '${orgId}', 'Macroeconomic Indicators', 'WPI, CPI, GDP calculation, ASI and PLFS analytics', NOW(), NOW())
ON DUPLICATE KEY UPDATE \`name\`=VALUES(\`name\`);
`;

  fs.writeFileSync("scripts/mysql_seed.sql", sql.trim());
  console.log("Created scripts/mysql_seed.sql successfully.");
}
